	...
	if ($username && $userid) {
		...
		if ($_POST['setcompletedatebtn']) {
			...
			if ($completedate) {
					...
					mysql_query("UPDATE `goal` SET `goal_complete` = '$completedate' WHERE `goal_id` = '$goalid'");
					
					//make sure the progress was added was changed
					$query = mysql_query("SELECT * FROM goal WHERE goal_id = '$goalid' AND goal_complete = '$completedate'");
					$numrows = mysql_num_rows($query);
					if ($numrows == 1) {
						echo "Your goal has been marked as completed.";
					}
					else 
						echo "An error has occured and your goal was not marked completed.";
		...
		echo "<form action='./completegoal.php?goalid=$goalid' method='post'>
		<table>
		<tr>
			<td><a href='./member.php'>Back to Homepage</a></td>
		</tr>
		<tr>
			<td><br />Goal completion date:</td>
